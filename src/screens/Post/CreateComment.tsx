...
export default function CreateComment({ navigation, route }: any) {
  ...
  const { username, pfp, title, text } = route.params

  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      try {
        const postResponse = await axios.get(`/${username}/${title}`)

        setAlreadyCommented(postResponse.data.userHasCommented)
      } catch (error: any) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setGifsLoading(true)
      try {
        let reqStr = `/v1/gifs/trending?api_key=${api_key}`

        if (gifSearch != "")
          reqStr = `/v1/gifs/search?q=${gifSearch}&api_key=${api_key}`

        const response = await axios({
          method: "get",
          url: reqStr,
          baseURL: "https://api.giphy.com", // Overrides the default base URL for this request
        })
        console.log(response.data.meta)

        setGifs(
          response.data.data.map((gifObj: any) => {
            return {
              name: gifObj.title,
              id: gifObj.id,
  ...
  const handleSendComment = async () => {
    setLoading(true)
    try {
      await axios.post(`/${username}/${title}/comment`, {
    ...
    return (
      <View style={styles.container}>
        <View style={styles.postContainer}>
          <TouchableOpacity
            onPress={() => navigation.navigate("UserInfo", { username })}
          >
        ...
        <View style={styles.commentContainer}>
          <Button
            disabled={loading}
            title="Delete your previous comment"
    ...
    <View style={styles.container}>
      ...
      <View style={styles.commentContainer}>
        ...
        <Button
          disabled={loading}
          title="Gifs"
          onPress={() => setOpenGifArea(!openGifArea)}
        ></Button>
        <Button
          disabled={loading}
          title="Send"
